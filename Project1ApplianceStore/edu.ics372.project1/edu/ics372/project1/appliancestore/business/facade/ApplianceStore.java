...
public class ApplianceStore implements Serializable {
	...
	private static BackOrderList backOrders = BackOrderList.getInstance();
	...
	public Result fulfillBackOrder(Request request) {
		...
		if (backOrder == null) {
			...
			if (backOrder.getQuantity() > models.search(backOrder.getAppliance().getId()).getQuantity()) {
				...
				SaleTransaction transaction = new SaleTransaction(backOrder.getCustomer(), backOrder.getAppliance(), number);
